...
writer = Agent(
    ...
    backstory='''You are a blog post writer who is capable of writing a linkedin blog post.
...
reviewer = Agent(
    ...
    backstory='''You are a professional article reviewer and very helpful for improving articles.
                 ...
                 You will provide enhanced blog points based on your knowlegde .''',
    goal="Improve the given article",
    llm=llm,
...
def StartCrew(prompt):
    ...
    task2 = Task(
        description=(
            "list review comments for improvement from the entire content of blog post to make it more better."
            "Make sure to check with a human if your comment is good before finalizing your answer."
            ),
        agent=reviewer,
        expected_output="improved blog article in form of text.",
        human_input=True,
    )
...
chat_interface.send("Enter the topic name for Blog!", user="System", respond=False)
